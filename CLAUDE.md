...
6. Publish workspace packages to npm using `NPM_TOKEN` (Automation token; never
   commit it). The CLI depends on core and tui, so publish in order:

   ```bash
   pnpm -F @xavier2code/llm-switch-core build
   pnpm -F @xavier2code/llm-switch-core publish --access public

   pnpm -F @xavier2code/llm-switch-tui build
   pnpm -F @xavier2code/llm-switch-tui publish --access public

   pnpm -F @xavier2code/llm-switch build
   pnpm -F @xavier2code/llm-switch publish --access public
   ```

   CI publishes automatically on tag pushes using the `NPM_TOKEN` repository
   secret.
