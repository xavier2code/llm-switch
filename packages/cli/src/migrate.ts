...
import { getConfigDir, TARGETS, type TargetConfig } from './config.js';
...
export async function ensureMigratedToCentralStore(
  ...
  const oldCentralDir = path.join(process.env.HOME ?? os.homedir(), '.config', 'llm-switch');
  ...
  for (const target of targets) {
    ...
    // the new store doesn't have one yet
